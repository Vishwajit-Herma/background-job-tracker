...
class RQIntegration:
    ...
    def __init__(
        ...
        Args:
            tracker (Tracker, optional): The Background Job Tracker client instance. If None, uses Tracker.get_instance().
            ...
            for mod in modules:
                ...
                if hasattr(mod, "__name__") and hasattr(mod, "__dict__"):
                    for name, obj in inspect.getmembers(mod, inspect.isfunction):
                        if getattr(obj, "__module__", None) == mod.__name__ and not name.startswith(
                            "_"
                        ):
                            discovered.add(f"{mod.__name__}.{name}")

        # 3. Queues (explicit or worker queues)
        queues_to_inspect = queues or (
            self.worker.queues if self.worker and hasattr(self.worker, "queues") else None
        )
