...
class TeamMemberViewSet(viewsets.ModelViewSet):
    ...
    def perform_destroy(self, instance):
        ...
        if instance.user == self.request.user:
            ...
            if not self.request.user.is_staff:
                current_member = TeamMember.objects.filter(
                    team=instance.team, user=self.request.user, is_active=True
                ).first()
    ...
    def perform_update(self, serializer):
        ...
        if (
            instance.role == "owner"
            and serializer.validated_data.get("role") != "owner"
    ...
    def perform_create(self, serializer):
        ...
        if not User.objects.filter(email=email).exists():
            raise ValidationError(
                {"email": "User does not have an account. They must register first."}
            )
        ...
        if TeamInvitation.objects.filter(
            team=team, email=email, status="pending", expires_at__gt=timezone.now()
        ).exists():
...
class MyInvitationsViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        return TeamInvitation.objects.filter(
            email=self.request.user.email, status="pending", expires_at__gt=timezone.now()
        ).select_related("team")
    ...
    def accept(self, request, pk=None):
        ...
        return Response(
            {"error": "Failed to accept invitation"}, status=status.HTTP_400_BAD_REQUEST
        )
